...
public class AdminController(IAdminService adminService) : ControllerBase
{
        ...
        [FromQuery] int pageSize = 10)
    {
        ...
        return Ok(new ApiResponse<PagedList<UserResponseDto>>(
            HttpStatusCode.OK, true, "Users retrieved successfully.", result));
    }
        ...
        [FromQuery] int pageSize = 10)
    {
        ...
        var result = await _adminService.GetDeletedUsersAsync(pageNumber, pageSize);
        var metadata = PaginationMetadata.From(result);
        Response.Headers.Append("X-Pagination", System.Text.Json.JsonSerializer.Serialize(metadata));

        return Ok(new ApiResponse<PagedList<UserResponseDto>>(
            HttpStatusCode.OK, true, "Deleted users retrieved successfully.", result));
    }
    ...
    public async Task<IActionResult> ForceLogout(string userId) => Ok(
        new ApiResponse<UserResponseDto>(
            HttpStatusCode.OK, true,
            $"User {userId} has been forcefully logged out.",
            await _adminService.ForceLogoutAsync(userId)));
    ...
    public async Task<IActionResult> RestoreUser(string userId) => Ok(
        new ApiResponse<UserResponseDto>(
            HttpStatusCode.OK, true,
            $"User {userId} has been restored successfully.",
            await _adminService.RestoreUserAsync(userId)));
}